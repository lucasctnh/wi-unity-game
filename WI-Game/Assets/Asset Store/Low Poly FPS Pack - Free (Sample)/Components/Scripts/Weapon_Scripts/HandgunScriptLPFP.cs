...
public class HandgunScriptLPFP : MonoBehaviour {
	...
	public Image crosshairType1;
	public Image crosshairType2;
	...
	private void Awake ()
	{
		...
		CheckChangeCrosshair();
	}

	private void CheckChangeCrosshair()
	{
		if (gameManager.crosshairType == 1)
		{
			crosshair = crosshairType1;
			crosshairType2.gameObject.SetActive(false);
		}
		else if (gameManager.crosshairType == 2)
		{
			crosshair = crosshairType2;
			crosshairType1.gameObject.SetActive(false);
		}

		crosshair.color = gameManager.selectedColor;
	}
	...
	private void LateUpdate () {
		//Weapon sway
		if (gameManager.isGameActive && !PauseMenu.gameIsPaused && weaponSway == true) {
	...
	private void Update () {
		...
		if (currentAmmo == 0)
		{
			...
			currentWeaponText.text = "NO AMMO";
			//Toggle bool
			outOfAmmo = true;
			//Auto reload if true
		...
		if (gameManager.isGameActive &&
			!PauseMenu.gameIsPaused &&
			(Input.GetKeyDown (KeyCode.R) || (outOfAmmo && Input.GetMouseButtonDown (0))) &&
			!isReloading &&
			!isInspecting &&
			!isAmmoFull)
		{
		...
		if (gameManager.isGameActive &&
			...
			(Input.GetKey (KeyCode.W) && !isRunning ||
